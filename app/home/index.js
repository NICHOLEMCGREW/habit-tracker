import {
  ...
  const index = () => {
	...
	const fetchHabits = async () => {
	  try {
		const response = await axios.get("http://localhost:3002/habitslist");
	...
	const handleCompletion = async () => {
	  try {
		...
		await axios.put(`http://localhost:3002/habits/${habitId}/completed`, {
	...
	const deleteHabit = async () => {
	  try {
		...
		const response = await axios.delete(
		  `http://localhost:3002/habits/${habitId}`
		);
